...
def plot_oi_zones(df, support_zones, resistance_zones): fig = go.Figure() fig.add_trace(go.Bar(x=df['strikePrice'], y=df['openInterest_CE'], name='Call OI', marker_color='red')) fig.add_trace(go.Bar(x=df['strikePrice'], y=df['openInterest_PE'], name='Put OI', marker_color='green')) for low, high in support_zones: fig.add_vrect(x0=low, x1=high, fillcolor="lightgreen", opacity=0.3, layer="below", line_width=0) for low, high in resistance_zones: fig.add_vrect(x0=low, x1=high, fillcolor="lightcoral", opacity=0.3, layer="below", line_width=0) fig.update_layout(title="OI Zones with Support & Resistance", barmode='group') st.plotly_chart(fig, use_container_width=True)

Store weights for scoring

weights = { "ChgOI_Bias": 2, "Volume_Bias": 1, "Gamma_Bias": 1, "AskQty_Bias": 1, "BidQty_Bias": 1, "IV_Bias": 1, "DVP_Bias": 1, }

def delta_volume_bias(price, volume, chg_oi): if price > 0 and volume > 0 and chg_oi > 0: return "Bullish" elif price < 0 and volume > 0 and chg_oi > 0: return "Bearish" elif price > 0 and volume > 0 and chg_oi < 0: return "Bullish" elif price < 0 and volume > 0 and chg_oi < 0: return "Bearish" else: return "Neutral"

def final_verdict(score): if score >= 4: return "Strong Bullish" elif score >= 2: return "Bullish" elif score <= -4: return "Strong Bearish" elif score <= -2: return "Bearish" else: return "Neutral"

def is_spot_within_zone(spot, zones): for low, high in zones: if low <= spot <= high: return (low, high) return None

def main(): try: now = datetime.now(timezone("Asia/Kolkata"))
...
headers = {"User-Agent": "Mozilla/5.0"}
    ...
    # Signal logic based on zone ranges
    signal_sent = False
