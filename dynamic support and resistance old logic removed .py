...
def detect_nearby_zones(df, spot, threshold=1.2, range_pts=100, vol_threshold=2000): nearby_df = df[df['strikePrice'].between(spot - range_pts, spot + range_pts)].copy()

nearby_df['ZoneTag'] = nearby_df.apply(
    lambda row:
        'Support' if row['openInterest_PE'] > threshold * row['openInterest_CE'] and row['totalTradedVolume_PE'] > vol_threshold else
        'Resistance' if row['openInterest_CE'] > threshold * row['openInterest_PE'] and row['totalTradedVolume_CE'] > vol_threshold else
        'Neutral', axis=1)
...
def main(): try: now = datetime.now(timezone("Asia/Kolkata")) headers = {"User-Agent": "Mozilla/5.0"} session = requests.Session() session.headers.update(headers) session.get("https://www.nseindia.com", timeout=5) url = "https://www.nseindia.com/api/option-chain-indices?symbol=NIFTY" response = session.get(url, timeout=10) data = response.json()
