...
def determine_level(row):
    if row['openInterest_PE'] > 1.5 * row['openInterest_CE']:
        return "Support"
    elif row['openInterest_CE'] > 1.5 * row['openInterest_PE']:
        return "Resistance"
    else:
        return "Neutral"
...
def analyze():
    try:
        ...
        if not signal_sent:
            send_telegram_message(
                f"📍 Spot: {underlying}\n{market_view}\nNo Signal — Market is {market_view}\nBias Score: {total_score} ({market_view})\nLevel: {atm_row['Level']}\nBiases: ChgOI: {atm_row['ChgOI_Bias']}, Volume: {atm_row['Volume_Bias']}, Gamma: {atm_row['Gamma_Bias']}, AskQty: {atm_row['AskQty_Bias']}, BidQty: {atm_row['BidQty_Bias']}, IV: {atm_row['IV_Bias']}, DVP: {atm_row['DVP_Bias']}, Strike: {atm_row['Strike']}"
            )
