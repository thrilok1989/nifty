...
def detect_nearby_zones(df, spot, threshold=1.2, range_pts=150, vol_threshold=2000):
    nearby_df = df[df['strikePrice'].between(spot - range_pts, spot + range_pts)].copy()

    nearby_df['ZoneTag'] = nearby_df.apply(
        lambda row:
            'Support' if row['openInterest_PE'] > threshold * row['openInterest_CE']
    ...
    support_zones = group_by_continuity(
        nearby_df[nearby_df['ZoneTag'] == 'Support']['strikePrice'].tolist()
    )
    resistance_zones = group_by_continuity(
        nearby_df[nearby_df['ZoneTag'] == 'Resistance']['strikePrice'].tolist()
    )
...
def main():
    try:
        ...
        # User-controlled zone parameters
        st.sidebar.title("⚙️ Zone Sensitivity")
        range_pts = st.sidebar.slider("±Strike Range from Spot", 50, 500, 150, step=50)
        threshold = st.sidebar.slider("OI Strength Threshold", 1.0, 2.0, 1.2)
        vol_threshold = st.sidebar.slider("Min Volume Threshold", 1000, 10000, 2000, step=500)
        ...
        col1.metric("📍 Spot Price", f"{underlying:.2f}")
        col2.metric("📅 Expiry", expiry)
        ...
        support_zones, resistance_zones = detect_nearby_zones(df, underlying, threshold, range_pts, vol_threshold)
        ...
        if zone_msg_parts:
            send_telegram_message("*Nifty Option Zones Alert*\n" + "\n".join(zone_msg_parts))
