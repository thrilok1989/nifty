...
def find_nearest_levels(df, spot):
    df_sorted = df.copy()
    df_sorted['Distance'] = abs(df_sorted['strikePrice'] - spot)
    support_df = df_sorted[df_sorted['Level'] == 'Support']
    resistance_df = df_sorted[df_sorted['Level'] == 'Resistance']
    nearest_support = support_df.sort_values('Distance').head(1)['strikePrice'].values[0] if not support_df.empty else 'N/A'
    nearest_resistance = resistance_df.sort_values('Distance').head(1)['strikePrice'].values[0] if not resistance_df.empty else 'N/A'
...
def analyze():
    try:
        ...
        nearest_support, nearest_resistance = find_nearest_levels(df, underlying)
        ...
        for row in bias_results:
            ...
            if row['Level'] == "Support" and row["BiasScore"] >= 2 and "Bullish" in market_view:
                option_type = 'CE'
            elif row['Level'] == "Resistance" and row["BiasScore"] <= -2 and "Bearish" in market_view:
                option_type = 'PE'
            else:
                continue
        ...
        st.markdown(f"**Nearest Support:** {nearest_support} | **Nearest Resistance:** {nearest_resistance}")
        if suggested_trade:
